...
def import_questions_preview():
    """预览 Excel 文件：返回各 sheet 名、列名、行数、前几行数据、自动列映射"""
    import pandas as pd
...
async def grade_answer():
    ...
    if question_id:
        q_for_strategy = get_question(int(question_id))
        if q_for_strategy and q_for_strategy.get('scoring_strategy'):
            scoring_strategy = q_for_strategy['scoring_strategy']
        else:
