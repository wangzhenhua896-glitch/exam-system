...
结构要求：
- 【题目信息】：完整复述题目内容和满分值
- 【标准答案要点】：核心要点 + 核心含义 + 关键词（至少2个）+ 等价表述（至少5个，覆盖同义替换/上下位词/口语化/部分匹配/相关概念）
- 【逐项评分规则】：每个得分点的得分条件、部分得分、不得分条件、易混淆判断
- 【反作弊规则】：复制题干=0分、空白=0分、答非所问=0分
...
4. 每份作答之间必须有明显差异：不同表述、不同得分点组合、不同错误模式、不同详细程度
5. 如果提供了【已有测试用例】区块，新生成的作答必须与每一条已有用例都明显不同
